...
class ORCParserTest extends AnyFlatSpec with Matchers with BeforeAndAfterAll {
  ...
  "ORCParser" should "read an ORC file into a DataFrame" in {
    // Given
    val orcData = Seq(
      Row("John Doe", 30L),
      Row("Jane Doe", 25L)
    ...
    val tempDir = java.nio.file.Files.createTempDirectory("ORCParserTest").toString
    ...
    // When
    val resultDf = ORCParser.read(spark, tempOrcPath)
  ...
  it should "throw an exception if the ORC file is malformed" in {
    ...
    assertThrows[org.apache.spark.sql.AnalysisException] {
      ORCParser.read(spark, tempOrcPath)
    }
  }
}
